...
def compute_eval_metrics(X, y_true, model, metrics):
    ...
    for metric_name in metrics:
        ...
        if metric_name == 'precision':
            score = precision_score(y_true, y_pred, zero_division=1)
        if metric_name == 'recall':
            score = recall_score(y_true, y_pred)
        if metric_name == 'accuracy':
            n_correct = np.sum(y_true.to_numpy().reshape(-1) == y_pred)
            score = n_correct / len(X)
...
def plot_roc_curve(X_train, X_val, y_train, y_val, trained_models, model_names):
    ...
    for model_name in model_names:
        ...
        for threshold in threshold_values:
            predictions_train = apply_threshold(probabilities_train, threshold)
            predictions_val = apply_threshold(probabilities_val, threshold)

            precision_train = precision_score(y_train, predictions_train, zero_division=1)
            precision_val = precision_score(y_val, predictions_val, zero_division=1)
        ...
        fig.add_trace(go.Scatter(x=train_recall_all, y=train_precision_all, name="Train"),
            row=i+1, col=1) # use enumerated value i to align figures vertically
        fig.add_trace(go.Scatter(x=val_recall_all, y=val_precision_all, name="Validation"),
            row=i+1, col=1) # use enumerated value i
    ...
    if (model_select):
        ...
        if 'eval_button_clicked' in st.session_state and st.session_state['eval_button_clicked']:
            ...
            if 'ROC Curve' in review_plot:
                trained_select = {model: st.session_state[model]
                                  for model in model_select}
                ##
                print(trained_select)
