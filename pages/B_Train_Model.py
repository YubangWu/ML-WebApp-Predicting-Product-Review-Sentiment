...
def train_sgdcv_classifer(X_train, y_train, model_name, params, cv_params, random_state=42):
    ...
    """
    sgd_cv_model_best_model_estimator = None
    # Add code here
    try:
        # write some code
        sgd_cv_model = GridSearchCV(
    ...
    sgd_cv_model_best_model_estimator = sgd_cv_model.best_estimator_
    return sgd_cv_model_best_model_estimator
...
def inspect_coefficients(trained_models):
    ...
    for name, model in trained_models.items():
        ...
        if model:
            ...
            st.write('Number of positive coefficients', np.sum(model.coef_ >= 0))
            st.write('Number of negative coefficients', np.sum(model.coef_ < 0))
            st.write('Total number of coefficients',len(model.coef_[0]))

    try:
        st.markdown('### cv_results' )
